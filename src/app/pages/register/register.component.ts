  ...
  submit(): void {
    ...
    this.survivorService.registerSurvivor(formData).subscribe({
      next: () => {
        ...
        const name = this.survivorForm.get('fullName')?.value;

        this.authService.login(idNumber, password).subscribe(success => {
          if (success) {
            // If provisional, we may not have an ID number, give them a temp one
            const loggedInUser = this.authService.currentUserValue;
            if (loggedInUser) {
              loggedInUser.name = name;
              loggedInUser.idNumber = idNumber === 'provisional' ? 'TEMP-ID-' + Math.floor(Math.random() * 10000) : idNumber;
              localStorage.setItem('survivor_user', JSON.stringify(loggedInUser));
            }
